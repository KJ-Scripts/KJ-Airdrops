...
RegisterNetEvent('KJAirdrop:PlaneDrop')
AddEventHandler('KJAirdrop:PlaneDrop', function(id)
	planeID = id
...
RegisterNetEvent('KJAirdrop:StartDropCrate')
AddEventHandler('KJAirdrop:StartDropCrate', function(id)
    ...
    lib.notify({
        id = 'crate_drop',
        title = 'AIRDROP',
        description = "Er is een vliegtuig gecrashed met wapens! Ga snel naar de blip op de map!",
        duration = 10000,
        position = 'top',
        style = {
        ...
        while crateID ~= nil do
            ...
            if dist < 200.0 then
                ...
                if dist < 3.0 and DoesEntityExist(CrateObject) and not isOpening then
                    ...
                    Draw3DText(objCoords.x, objCoords.y, objCoords.z, '[~b~E~w~] Airdrop Openen')
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        if isProgSuccess then
                            isOpening = false
                            RemoveCrateBlip()
                            TriggerServerEvent('KJAirdrop:PickUpCrate', crateID)
...
function CreateCrateBlip(BUGOKAHAH)
    RemoveCrateBlip()
	CrateBlip2 = AddBlipForRadius(BUGOKAHAH.x, BUGOKAHAH.y, BUGOKAHAH.z, 200.0) -- Hoe groot de zone moet zijn
...
RegisterNetEvent('KJAirdrop:ResetCrate')
AddEventHandler('KJAirdrop:ResetCrate', function(id)
...
RegisterNetEvent('KJAirdrop:AddExplosion')
AddEventHandler('KJAirdrop:AddExplosion', function()
	AddExplosion(crateID.x, crateID.y, crateID.z + 1.00, 40, 0.00, true, false, 1.00)		
end)

RegisterNetEvent('KJAirdrop:smoke')
AddEventHandler('KJAirdrop:smoke', function()
